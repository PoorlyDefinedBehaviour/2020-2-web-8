...
context("Register museum work form", () => {
  ...
  context("when work name is not valid, should show an error message", () => {
    it("when work name is empty", () => {
      ...
      cy.get("[data-test-id=name]")
        ...
        .get("span")
        .should("be.visible")
        .and("contain", "O nome da obra deve ser informado")
    })
  ...
  context("when author name is not valid, should show an error message", () => {
    it("when author name is empty", () => {
      ...
      cy.get("[data-test-id=name]")
        ...
        .get("span")
        .should("be.visible")
        .and("contain", "O nome do autor deve ser informado")
    })
    ...
    () => {
      ...
      it("when release year is not an integer", () => {
        ...
        cy.get("[data-test-id=releaseYear]")
          ...
          .get("span")
          .should("be.visible")
          .and("contain", "O ano da obra deve ser um número inteiro")
      })
      ...
      it("when release year is not a four digit integer", () => {
        ...
        cy.get("[data-test-id=releaseYear]")
          ...
          .get("span")
          .should("be.visible")
          .and("contain", "O ano da obra deve estar no formato AAAA")
      })

      it("when release year is in the future", () => {
        cy.get("[data-test-id=releaseYear]").type(
          faker.random.number({ min: new Date().getFullYear() + 1 })
        )
        ...
        cy.get("[data-test-id=releaseYear]")
          ...
          .get("span")
          .should("be.visible")
          .and("contain", "O ano da obra não pode ser no futuro")
      })
    }
  )
  ...
  context("after form is filled correctly", () => {
    ...
    context("when deleting a row", () => {
      it("when user cancels deletion, nothing should happen", () => {
        fillFormAndSubmit()
      ...
      it("when user confirms deletion, should delete work", () => {
        fillFormAndSubmit()
